...
private:
  ...
  ros::Publisher markers_rviz_pub;
  ...
  int idExists(const int id, const my_ur10_msgs::ArucoMarkerArray& markers) {
    int count = 0;
...
public:
      ...
      it(nh)
  {
    ...
    process_cmd_sub = nh.subscribe("/aruco_multiple/cmd", 1, &ArucoMultiple::process_cmd_callback, this);
    ...
    image_detector_pub = it.advertise("result", 1);
    markers_pose_pub = nh.advertise<my_ur10_msgs::ArucoMarkerArray>("markers_pose", 1);
    markers_rviz_pub = nh.advertise<visualization_msgs::MarkerArray>("markers_rviz", 1);
    ...
    dyn_rec_server.setCallback(boost::bind(&ArucoMultiple::reconf_callback, this, _1, _2));
  }
  ...
  void image_callback(const sensor_msgs::ImageConstPtr& msg)
  {
    ...
    if ((image_detector_pub.getNumSubscribers() == 0) &&
        ...
        (markers_rviz_pub.getNumSubscribers() == 0))
    {
    ...
    try {
      ...
      my_ur10_msgs::ArucoMarkerArray marker_array; 

      // Init visualization_msgs::MarkerArray msg
      ...
      for (size_t i = 0; i < markers.size(); ++i) {
        ...
        int c = idExists(markers[i].id, marker_array);
        if (c > 0) {
        ...
        my_ur10_msgs::ArucoMarker marker_msg;
        ...
        visMarker.header.frame_id = reference_frame;
        visMarker.id = markers[i].id;
        visMarker.type   = visualization_msgs::Marker::CUBE;
        visMarker.action = visualization_msgs::Marker::ADD;
        visMarker.pose = marker_msg.pose;
        ...
        marker_msg.mark_rviz = visMarker;


        marker_array.markers.push_back(marker_msg);
        markers_rviz.markers.push_back(visMarker);
      ...
      markers_rviz_pub.publish(markers_rviz);
      markers_pose_pub.publish(marker_array);
...
{
  ros::init(argc, argv, "aruco_multiple");
