    ...
    for(std::size_t d = 0; d < ids.size(); ++d){
      if(a_array.markers[i].id == ids[d]){
        for(std::size_t j = 0; j < a_array.markers[i].img_points.size(); ++j){
          result.push_back(a_array.markers[i].img_points[j].x);
          result.push_back(a_array.markers[i].img_points[j].y);
        }
      }
    }
  }
...
Eigen::MatrixXd obtainIteractionMatrix(std::vector<double> p, std::vector<double> Z){
  Eigen::MatrixXd result(p.size(),6);
  int cnt_Z = 0;
  for(std::size_t i = 0; i < p.size(); i = i + 2){
    result(i, 0) = - 1.0 / Z[cnt_Z];
    result(i, 1) = 0;
    result(i, 2) = p[i] / Z[cnt_Z];
    result(i, 3) = p[i] * p[i+1];
    result(i, 4) = -(1 + (p[i] * p[i]));
    result(i, 5) = p[2];
    result(i+1, 0) = 0;
    result(i+1, 1) = -1.0 / Z[cnt_Z];
    result(i+1, 2) = p[i+1] / Z[cnt_Z];
    result(i+1, 3) = (1 + (p[i+1] * p[i+1]));
    result(i+1, 4) = - p[i] * p[i+1];
    result(i+1, 5) = - p[i];
    cnt_Z++;
  }
...
int main(int argc, char** argv)
{
  ...
  std::vector<double> joint_values, actual_rpy, old_actual_rpy, diff_c_s;
  ...
  while(!keep){
    if(see_aruco){
      ...
      L = obtainIteractionMatrix(c_s, Z);
      L_pi = PseudoinverseMoorePenrose(L, false);

      diff_c_s = diff_Vector(c_s, d_c_s);
      diff_E = toEigenVector(diff_c_s);
      vel_camera = L_pi * diff_E;
      vel_camera = applyGains(gains, vel_camera);

      keep = BadGoalPosition(diff_c_s, m_error);
